...
(* Read an existing .env file and return values for known keys *)
procedure ReadExistingEnv( const EnvPath: string; var MachineName, HaUrl, HaToken: string );
var
...
begin
  ...
  begin
    Val := ParseEnvValue( Lines[I], 'MACHINE_NAME' );
    if Val <> '' then MachineName := Val;

    Val := ParseEnvValue( Lines[I], 'HA_URL' );
    if Val <> '' then HaUrl := Val;

    Val := ParseEnvValue( Lines[I], 'HA_TOKEN' );
    if Val <> '' then HaToken := Val;
  end;
end;
...
var
  MachineName, HaUrl, HaToken, EnvPath: string;
begin
  ...
  HaUrl := 'http://homeassistant.local:8123/api/states/';
  HaToken := '';

  (* Read existing .env if upgrading *)
  EnvPath := ExpandConstant( '{autopf}\EleksWFD Monitor\.env' );
  ReadExistingEnv( EnvPath, MachineName, HaUrl, HaToken );

  ConfigPage.Values[0] := MachineName;
  ConfigPage.Values[1] := HaUrl;
  ConfigPage.Values[2] := HaToken;
end;
