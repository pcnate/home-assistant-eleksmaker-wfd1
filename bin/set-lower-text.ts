...
async function main() {
  const res = await fetch( `${ HA_URL }/api/states/input_text.eleksmaker_lower`, {
    method: 'POST',
    headers: {
    ...
    body: JSON.stringify({
      state: text,
      attributes: { friendly_name: 'EleksMaker Lower', icon: 'mdi:clock-digital', min: 0, max: 255 },
    }),
  });

  if ( !res.ok ) throw new Error( `HA POST failed: ${ res.status }` );
  console.log( text ? `Lower text set: "${ text }"` : 'Lower text cleared (clock mode)' );
}
